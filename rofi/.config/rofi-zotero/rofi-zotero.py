...
From: https://github.com/hanschen/rofi-zotero/
Modified to include item type in display and change prompt to Zotero.
...
_QUERY_TITLES = """
    SELECT parentInfo.itemID, parentItemDataValues.value, attachmentInfo.key
...
_QUERY_DATES = """
    SELECT parentInfo.itemID, parentItemDataValues.value
    FROM itemAttachments
         INNER JOIN items AS attachmentInfo
                 ON attachmentInfo.itemID = itemAttachments.itemID
         INNER JOIN itemData AS attachmentItemData
...
def parse_args():
    ...
    parser.add_argument("-p", "--profile", type=str, default=None, help="name of the Zotero profile to use. Default: The default Zotero profile")
    parser.add_argument("-l", "--list", action="store_true", default=False, help="print out list of results instead of passing it to Rofi")
    parser.add_argument("--rofi-args", type=str, default=DEFAULT_ROFI_ARGS, help=f"arguments to Rofi, usually given as: --rofi-args=\"-i -t theme -p title\\ with\\ space\". Default: \"{DEFAULT_ROFI_ARGS}\"")
    parser.add_argument("--viewer", type=str, default=DEFAULT_VIEWER, help=f"application to open attachments, with \"%%u\" to indicate where the path should go. Default: \"{default_viewer_without_percent}\"")
    parser.add_argument("--prompt-paper", type=str, default=DEFAULT_PROMPT_PAPER, help=f"prompt title when searching through papers. Default: \"{DEFAULT_PROMPT_PAPER}\"")
    parser.add_argument("--prompt-attachment", type=str, default=DEFAULT_PROMPT_ATTACHMENT, help=f"prompt title when searching through attachments. Default: \"{DEFAULT_PROMPT_ATTACHMENT}\"")
    parser.add_argument("-c", "--config", type=Path, default=DEFAULT_ZOTERO_CONFIG, help=f"path to Zotero config directory. Default: \"{DEFAULT_ZOTERO_CONFIG}\"")
...
def main(
    ...
    try:
        ...
        print("'rofi' command not found, make sure it is installed and that it is in PATH", file=sys.stderr)
        return

    if not config.exists():
        show_error(f"Zotero config directory not found: {config}\nUse the --config option to specify the path to the config directory.")
        return
    try:
    ...
    all_item_types = get_item_info(database_copy, _QUERY_ITEM_TYPES)
    ...
    authors = {id: format_authors(author_list) for id, author_list in author_lists.items()}
    ...
    item_types = {}
    for item_id, item_type in all_item_types:
    ...
    if len(files_to_open) == 1:
        ...
        files_input = "\n".join(files)
        rofi_command = (["rofi", "-dmenu", "-format", "i"] + rofi_args + ["-p", prompt_attachment])
        rofi = subprocess.run(rofi_command, capture_output=True, text=True, input=files_input)
        selected_index = rofi.stdout.strip()
        if not selected_index:
            return
